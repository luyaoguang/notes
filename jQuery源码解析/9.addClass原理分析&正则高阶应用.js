...
(function(root) {
	...
	jQuery.extend({
		...
		access: function(elems, func, key, value) { //text  css
			...
			var testing = key === null; //判断是否为html/text 操作
			...
			if (value !== undefined) { //如果value有值
				chain = true;
				if (testing) {//如果key无值 + value有值 一定是 text|html 的set操作
	...
	jQuery.fn.extend({
		...
		css: function(key, value) { // key === string  key === array  key  === object
			return jQuery.access(this, function(key, value) {
				...
				if (jQuery.isArray(key)) { //获取当前元素的css样式表达  this   style
					styles = window.getComputedStyle(this, null);//拿到所有css样式
					len = key.length;
					for (var i = 0; i < len; i++) {
        ...
        addClass: function(value) {
            var len = this.length;//拿到实例对象DOM元素长度
            var proceed = typeof value === "string" && value;
            var classes,//保存所有class类名
                elem,
                clazz,
                cur,
                i = 0,
            ...
            if(proceed) {
                ...
                for(; i < len; i++) {//遍历所有的类名
                    elem = this[i];//找到每个DOM元素
                    cur = elem.nodeType === 1 && (elem.className ? (" "+ elem.className +" ") : " ");//找到DOM元素的className
                    if(cur) {//如果存在className
                        while(clazz = classes[j++]) {
                            if(cur.indexOf(" "+ clazz +" ") < 0) {//如果当前DOM元素没有对应类名
                                cur += clazz + " ";
                            }
                        }
                        elem.className = cur.trim();//重新赋值
                    }
                }
            }
        }
